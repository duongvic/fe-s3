...
import { ExtendReviewInstance } from './Extend';
import {
  selectContract,
  // selectLoading,
  selectOrder,
...
export default function ExtendOrder({ orderType }: Props) {
  ...
  useEffect(() => {
    dispatch(orders.actions.setItemsVmConfig([]));
    if (order) {
  ...
  useEffect(() => {
    if (noticeOrder === t('Message.EXTEND_SUCCESS')) {
      ...
      <Row gutter={8}>
        <Col span={12}>
          <Card
            ...
            style={{ height: '100%' }}
          >
            <UserFormBase mode={MODE_EXTEND} userType={user?.user_type} />
          </Card>
        </Col>
        <Col span={12}>
          <Card
            ...
            style={{ height: '100%' }}
          >
            <ContractFormBase mode={MODE_EXTEND} />
          </Card>
        </Col>
      </Row>
